...
void imprimirDados(Livro * l1){
	...
	switch (l1->area){
		case 0: printf("Area: Computacao\n");
			break;
		case 1: printf("Area: Literatura\n");
			break;
		case 2: printf("Area: Ciencias\n");
			break;
		case 3: printf("Area: Artes\n");
			break;
		case 4: printf("Area: Geral\n");
			break;	
	}

	printf("Ano publicacao: %d\n", l1->anopub);
	printf("Numero de emprestimos: %d\n", l1->noEmprestimos);
	if(l1->emprestado == true)
...
void ordenaBibliotecaPorNumeroEmprestimos(Biblioteca * bib){
	int i, fim;
	Livro* aux;
	for(fim = bib->posLivre - 1; fim > 0; fim--){
		for(i = 0; i < fim; i++){
...
void ordenaBibliotecaNomeLivro(Biblioteca * bib)
{
	...
	for(fim = bib->posLivre - 1; fim > 0; fim--){
		for(i = 0; i < fim; i++){
			int result = strcmp(bib->livros[i]->nome, bib->livros[i + 1]->nome);
			if(result > 0){
				aux = bib->livros[i];
				bib->livros[i] = bib->livros[i + 1];
				bib->livros[i + 1] = aux;
...
Livro * buscaLivro(Biblioteca * bib, char * nNomeLivro) //2
{
	int i, resultado;
	for(i = 0; i < bib->posLivre; i++){
		resultado = strcmp(bib->livros[i]->nome, nNomeLivro);
		if (resultado == 0)
			return bib->livros[i];
	}
	return NULL;
...
void insereLivro(Biblioteca *  bib, Livro * l)  //1
{
	if(bib->posLivre >= 0 && bib->posLivre < TAM){
		bib->livros[bib->posLivre] = l;
		bib->posLivre++;
	}
	ordenaBibliotecaPorNumeroEmprestimos(bib);
	int i;
	for(i = bib->posLivre - 1; i >= 0; i--){
...
para o nEmprestador. A função retorna o ponteiro para o livro emprestado
...
Livro * emprestaLivro(Biblioteca * bib, char * nNomeLivro, char * nEmprestador) //3
{
	int i;
	for(i = 0; i < bib->posLivre; i++){
		if(strcmp (bib->livros[i]->nome, nNomeLivro) == 0 && bib->livros[i]->emprestado == false){
			bib->livros[i]->emprestado = true;
			strcpy(bib->livros[i]->nomeDoEmprestador, nEmprestador);
			bib->livros[i]->noEmprestimos++;
			return bib->livros[i];
		}
	}
	return NULL;
...
Livro * devolveLivro(Biblioteca * bib, char * nNomeLivro)
{
	int i;
	for(i = 0; i < bib->posLivre; i++){
		if(strcmp (bib->livros[i]->nome, nNomeLivro) == 0 && bib->livros[i]->emprestado == true){
			bib->livros[i]->emprestado = false;
			strcpy(bib->livros[i]->nomeDoEmprestador, "");
			return bib->livros[i];
		}
	}
	return NULL;
